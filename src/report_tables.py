...
REPORTED_ABLATIONS = (
    ...
    "touching: width excess below 4.5 px (previous rule)",
    "no bright-background test",
    "speck threshold 0.3 (previous)",
    "no dark-region test",
    "no recovery of erased grains",
...
def ablation_table():
    ...
    labels = {
        ...
        "touching: width excess below 4.5 px (previous rule)":
            r"米粒短轴不足 4.5 像素时改用厚度判据判粘连，即修正前的写法",
        "no bright-background test": r"去掉亮背景判据",
        "speck threshold 0.3 (previous)": r"碎屑门限取 0.3，即修正前的写法",
        "no dark-region test": r"去掉暗区判据",
        "no recovery of erased grains": r"不找回被开运算抹掉的米粒",
